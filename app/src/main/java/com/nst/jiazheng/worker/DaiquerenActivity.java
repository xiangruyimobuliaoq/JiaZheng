 ...
 * 更新描述
 */
@Layout(layoutId = R.layout.activity_daiqueren)
public class DaiquerenActivity extends BaseToolBarActivity {
    @BindView(R.id.tx)
    CircleImageView tx;
    ...
    class PicAdapter extends BaseQuickAdapter<UpFile, BaseViewHolder> {
        ...
        protected void convert(BaseViewHolder baseViewHolder, UpFile upfile) {
            ...
            try {
                Glide.with(DaiquerenActivity.this).load(upfile.path).centerCrop().into(pic);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
