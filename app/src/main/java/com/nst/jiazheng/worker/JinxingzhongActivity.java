 ...
 * 更新描述
 */
@Layout(layoutId = R.layout.activity_jinxingzhong)
public class JinxingzhongActivity extends BaseToolBarActivity {
    @BindView(R.id.tx)
    CircleImageView tx;
    ...
    private void endOrder(String id) {
    ...
    class PicAdapter extends BaseQuickAdapter<UpFile, BaseViewHolder> {
        ...
        protected void convert(BaseViewHolder baseViewHolder, UpFile upfile) {
            ...
            try {
                Glide.with(JinxingzhongActivity.this).load(upfile.path).centerCrop().into(pic);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
